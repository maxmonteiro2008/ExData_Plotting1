...
png(filename = "./data/plot4.png",480,480) # opening the png device
# Setting parameters
par(mfrow = c(2, 2), mar = c(4, 4, 4, 4), oma = c(2, 2, 2, 2))

plot(data$"Join_time",data$Global_active_power,type ="l",ylab ="Global Active Power",xlab = "" )
plot(data$"Join_time",data$Voltage,type ="l",ylab ="Voltage",xlab = "datetime" )
...
with(data,points(data$Join_time,data$Sub_metering_1, type = "l",ylab = "Energy sub metering",xlab = ""))
...
legend("topright",lwd=1,cex =0.9,xjust=1,yjust=1,x.intersp =.075  ,text.width=100000,inset =.02,lty=1,bty="n",col=c("black","red","blue"), legend=leg )

plot(data$"Join_time",data$Global_reactive_power,type ="l",ylab ="Global_reactive_power",xlab = "datetime" )
